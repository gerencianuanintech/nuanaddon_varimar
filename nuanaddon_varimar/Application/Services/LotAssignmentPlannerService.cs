...
namespace nuanaddon_varimar.Application.Services {
    public class LotAssignmentPlannerService : ILotAssignmentPlannerService {
        ...
        public LotAssignmentPlannerService()
            : this(new LotSelectionService()) {
        }
        ...
        public IReadOnlyList<BatchAssignmentDto> BuildAssignments(
            ...
            DateTime baseDate = BatchAssignmentCustomerService.CalculateBaseDate(deliveryDate, safeConfig);

            List<AvailableBatchDto> customerRuleBatches = availableBatches
                .Where(batch => BatchAssignmentCustomerService.BatchPassesCustomerRule(cardCode, batch, baseDate))
                .ToList();

            return lotSelectionService.BuildAssignments(itemCode, requiredQuantity, customerRuleBatches, safeConfig);
        }
    }
}
